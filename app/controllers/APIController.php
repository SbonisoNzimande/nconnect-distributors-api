	...
	static public $app_controller;
		...
		public function get($request) {
			...
			switch ($subRequest) {
				...
				case 'GetProductCategories': 
					$categories   		= self::validate_get_product_categories ();
				...
				case 'GetProductsByCategory': 
					$category_id 		= $request->parameters ['category_id'];
					$categories   		= self::validate_get_products ($category_id);
				...
				case 'CheckIfScanned': 
					...
					$store_id   		= $request->parameters ['store_id'];
					$category_id   		= $request->parameters ['category_id'];
					$rass_id    		= $request->parameters ['rass_id'];
					$product_id    		= $request->parameters ['product_id'];


					$promos   			= self::validate_check_if_scanned (
											$rep_id,
											$store_id,
											$category_id,
											$rass_id,
		...
		public function post ($request) {
			...
			switch ($subRequest) {
				...
				case 'SaveDeviceID': 
					// $rep_id         = $request->parameters ['rep_id'];
					// $device_token   = $request->parameters ['device_token'];

					$params 		= json_decode (file_get_contents('php://input'),true);

					$check   		= self::validate_save_device(
										$params
									);
				...
				case 'OrderProducts': 
					...
					$check   		= self::validate_order_product(
										$params
									);
				...
				case 'SaveRangingAssortment': 
					...
					// $category_id 	= $request->parameters ['category_id'];
					// $rass_id 		= $request->parameters ['rass_id'];
					// $barcode_number = $request->parameters ['barcode_number'];

					$check   		 = self::validate_rang_ass(
										$params
									 );
		...
		static protected function get_rep_data (
			...
			$notifications 	= self::$app_controller->get_notification_by_repid ($rep_id);
			...
			$percentage_text    =  $check_current .' out of ' . $call_target . ' stores';

			$rep_promos 		= self::$app_controller->get_reps_promos ($rep_id);
			$reps 				= self::$app_controller->get_rep_by_repid ($rep_id);
			...
			$region 			= $reps['region'];
			$bin_field			= preg_replace('/[^A-Za-z0-9]/', "_", strtolower($region)) .'_bins';
			...
			$return_data 		= array('target_week' => $target_week, 'percentage_text' => $percentage_text, 'notification' => $notification, 'promo_info' => $return_arr);
		...
		static protected function validate_check_if_scanned (
										$rep_id,
										$store_id,
										$category_id,
										$rass_id,
			...
			$scanned_check 		= self::$app_controller->check_if_already_scanned ($rep_id, $store_id, $category_id, $rass_id, $product_id);

			if (!is_array($scanned_check) && count($scanned_check) == 0) {
				return array('success'=>false, 'text'=>'Not Scanned');
			}

			return array('success'=>true, 'text'=>'Not Scanned');
		...
		static protected function validate_get_store_promos (
			...
			$object 			= self::$app_controller->get_open_promos ($rep_id, $store_id);
		...
		static protected function validate_get_pos_details (
			...
			foreach ($poss as $p) {
				$return[] = array(
						...
						"pos_upload_image"=> 'http://distributors.nconnectapp.co.za/files/PictureOfSuccess/' . $p["pos_upload_image"],
		...
		static protected function validate_get_product_list (
			...
			$submissions			= self::$app_controller->get_randass_submitions ($category_id, $store_id, $rep_id);
			$return_array 		 	= array();
			...
			foreach ($products as $p) {
				...
				$submissions_filtered 	= self::$app_controller->filter_by_value ($submissions, 'product_id', $p["product_id"]);

				$is_scanned 	= false;
				foreach ($submissions_filtered as $s) {
					$scanned 				= $s['scanned'];
		...
		static public function get_npd_survey_list ($store_id) {
			...
			$store_list   	= self::$app_controller->get_stores_information ();
			$survey_list    = self::$app_controller->get_npds ();
		...
		static protected function validate_get_product_categories() {


			$categories 		= self::$app_controller->get_product_categories ();
		...
		static protected function validate_get_products ($category_id) {


			$products 		= self::$app_controller->get_products_by_category ($category_id);

			$return 	= [];
			$dir 		= 'http://distributors.nconnectapp.co.za/images/';
			foreach ($products as $p) {
				$return[] =[
					'id' => $p['id'],
					'productCode' => $p['productCode'],
					'productName' => $p['productName'],
					'productPrice' => (!is_numeric($p['productPrice'])? 0: $p['productPrice']),
					'productLowestPrice' => (!is_numeric($p['productLowestPrice'])? 0: $p['productLowestPrice']),
					'productImage' => $dir.$p['productImage'],
					'Qty' => 0,
		...
		static protected function validate_order_product (
										$params
									) {
			...
			if (empty($store_id)) {
				return array('success'=>false, 'text'=>'Invalid Store ID');
			}

			if (empty($order_total)) {
				return array('success'=>false, 'text'=>'Order Total Is Empty');
			}

			if (empty($number_products)) {
				return array('success'=>false, 'text'=>'Select Atleast One Product');
			}

			$products_ordered = json_encode($order_products);

			// var_dump(json_encode($order_products));
			// die();
			


			$save 			= self::$app_controller->save_new_order ($rep_id, $store_id, $order_total, $number_products, $products_ordered);

			if ($save === true) {
				return array('success'=>true, 'text' => 'Successfuly Orderd');
			}else{
				return array('success'=>false, 'text' => 'Error Ordering, ' .$save);
			}
		...
		static protected function validate_strikerate(
			...
			$dir 						= '../distributors.nconnectapp.co.za/images/';
		...
		static protected function validate_rang_ass(
										$params
									 ) {
			...
			$product 			= self::$app_controller->get_product_by_barcode ($barcode_number, $category_id);
			if (!is_array($product) OR (count($product) < 1)) {
				return array('success'=>false, 'text'=>'Product Does\'t exist in this category');
			}
			...
			if (!isset($category_id)) {
				return array('success'=>false, 'text'=> 'Invalid Category');
			}

			if (!isset($rass_id)) {
				return array('success'=>false, 'text'=> 'Invalid Rand Ass ID');
			}

			$scanned_check = self::$app_controller->check_if_already_scanned ($rep_id, $store_id, $category_id, $rass_id, $product_id);

			if (count($scanned_check) >= 1) {
				return array('success'=>false, 'text'=>'Product Already Scanned');
			}

			
			$save 			= self::$app_controller->save_new_rang_ass ($rep_id, $store_id, $category_id, $rass_id, $product_id);
		...
		static protected function validate_survey_upload(
			...
			$dir 						= '../distributors.nconnectapp.co.za/files/' .$survey_type_name .'/'. $survey_id .'/';
		...
		static protected function validate_pos_upload(
			...
			$dir 				= '../distributors.nconnectapp.co.za/files/PictureOfSuccess/';
		...
		static protected function validate_save_device(
										$params
									) {
			...
			if(empty($device_token)){
				return array('success'=>false, 'text'=>'Invalid Device Token');
			}


			$chekout 		= self::$app_controller->save_rep_device_token ($rep_id, $device_token);

			if (!$chekout) {
				return array('success'=>false, 'text'=>'Saving Device Token');
			}
