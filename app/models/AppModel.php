...
abstract class AppModel 
{
	 ...
	 * @return 
	 */
	static public function get_products_by_category_db ($category_id) {

		try{
			$statement 	= self::$mysqli->prepare("SELECT * FROM products WHERE categoryId = :category_id");
	 ...
	 * @return 
	 */
	static public function get_product_categories_db () {

		try{
			$statement 	= self::$mysqli->prepare("SELECT * FROM product_categories");
			$statement->execute();
	 ...
	 * @return 
	 */
	static public function get_device_by_token_db ($device_token) {
	 ...
	 * @return 
	 */
	static public function get_product_by_barcode_db ($barcode, $category_id) {

		static $statement = null;

		try{
			$query    	= "SELECT * FROM rand_ass_products WHERE sku = :sku AND category_id = :category_id";

			$statement 	= self::$mysqli->prepare($query);

			$assoc 		= array (":sku" => $barcode, ":category_id" => $category_id);
			// $exQuery=str_replace(array_keys($assoc), array_values($assoc), $query);
			// echo $exQuery;
			$statement->execute ($assoc);
	 ...
	 * @return 
	 */
	static public function get_randass_submitions_db ($category_id, $store_id, $rep_id) {
	 ...
	 * @return 
	 */
	static public function get_npds_db () {
	 ...
	 * @return 
	 */
	static public function save_new_pos_db ($rep_id, $store_id, $category_id, $category_inline, $pos_id, $image){
	 ...
	 * @return 
	 */
	static public function save_new_rang_ass_db  ($rep_id, $store_id, $category_id, $rass_id, $product_id){

		static $statement 	= null;

		try{
			$statement 		= self::$mysqli->prepare("INSERT INTO ranging_assortment_submission (
														  store_id,
														  rep_id,
														  product_id,
														  category_id,
	 ...
	 * @return 
	 */
	static public function check_if_already_scanned_db  ($rep_id, $store_id, $category_id, $rass_id, $product_id) {
	...
	static public function get_rep_by_repid_db ($rep_id) {
		...
		try{
			...
			$query    	= "SELECT * FROM reps WHERE rep_id = :rep_id AND status = :status";

			$statement 	= self::$mysqli->prepare($query);

			$assoc 		= array(":rep_id" => $rep_id, ":status" => 'A');
			// $exQuery=str_replace(array_keys($assoc), array_values($assoc), $query);
			// echo $exQuery;
			$statement->execute($assoc);
	 ...
	 * @return 
	 */
	static public function get_notification_by_repid_db ($rep_id) {

		static $statement = null;

		try{
			$statement 	= self::$mysqli->prepare("SELECT * FROM notifications ORDER BY created");
	 ...
	 * @return 
	 */
	static public function save_rep_device_token_db ($rep_id, $device_token){
		static $statement = null;

		try{

			$query 		= "UPDATE reps SET pushId = :device_token WHERE `rep_id` = :rep_id";

			$statement 	= self::$mysqli->prepare($query);

			$assoc 		= [":rep_id" => $rep_id, ":device_token" => $device_token];
			// $exQuery=str_replace(array_keys($assoc), array_values($assoc), $query);
			// echo $exQuery;
			$statement->execute ($assoc);
	 ...
	 * @return 
	 */
	static public function save_new_order_db ($rep_id, $store_id, $order_total, $number_products, $products_ordered) {
